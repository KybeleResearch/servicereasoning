...
import ASD.diagram.edit.commands.ProfileRefersCreateCommand;
import ASD.diagram.edit.commands.ProfileRefersReorientCommand;
...
public class OperationItemSemanticEditPolicy extends
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (ASDVisualIDRegistry.getVisualID(incomingLink) == ProfileRefersEditPart.VISUAL_ID) {
				...
				for (Iterator<?> cit = node.getChildren().iterator(); cit
					...
					case MessageEditPart.VISUAL_ID:
						for (Iterator<?> it = cnode.getSourceEdges().iterator(); it
							...
							if (ASDVisualIDRegistry.getVisualID(outgoingLink) == MessageInfoTypeEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(
	...
	protected Command getStartCreateRelationshipCommand(
		...
		if (ASDElementTypes.ProfileRefers_4003 == req.getElementType()) {
			return null;
		}
		return null;
	}
	...
	protected Command getCompleteCreateRelationshipCommand(
			CreateRelationshipRequest req) {
		if (ASDElementTypes.ProfileRefers_4003 == req.getElementType()) {
			return getGEFWrapper(new ProfileRefersCreateCommand(req,
	...
	protected Command getReorientReferenceRelationshipCommand(
		...
		case ProfileRefersEditPart.VISUAL_ID:
			return getGEFWrapper(new ProfileRefersReorientCommand(req));
		}
