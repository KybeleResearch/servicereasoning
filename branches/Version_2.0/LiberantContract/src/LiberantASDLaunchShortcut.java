...
public class LiberantASDLaunchShortcut implements ILaunchShortcut {
	
	public static final String MODEL_CONFIGTYPE_ID = Activator.PLUGIN_ID + ".LiberantASD_lct";
	...
	protected ILaunchConfiguration createConfiguration(String pathIN) {
		...
		try {
			...
			wc.setAttribute("outModel", new Path(pathIN).removeFileExtension().toOSString() + "-lib.xmi");
			config = wc.doSave();
