...
import liberantcontract.Activator;;

public class GenerateLiberantContract {
	/**
	...
	private void loadProperties() throws Exception {
		...
		properties.load(getFileURL("LiberantASD.properties").openStream());
		Resource.Factory.Registry.INSTANCE.getExtensionToFactoryMap().put("ecore", new EcoreResourceFactoryImpl());	
	}
	
	protected InputStream[] getModulesList() throws IOException {
		InputStream[] modules = null;
		String modulesList = properties.getProperty("LiberantASD.modules");
		if (modulesList != null) {
	...
	protected String getMetamodelUri(String metamodelName) {
		return properties.getProperty("LiberantASD.metamodels." + metamodelName);
	}
	
	protected Map<String, Object> getOptions() {
		Map<String, Object> options = new HashMap<String, Object>();
		for (Entry<Object, Object> entry : properties.entrySet()) {
			if (entry.getKey().toString().startsWith("LiberantASD.options.")) {
				options.put(entry.getKey().toString().replaceFirst("LiberantASD.options.", ""), 
	...
	protected static URL getFileURL(String fileName) throws IOException {
		final URL fileURL;
		if (isEclipseRunning()) {
			URL resourceURL = GenerateLiberantContract.class.getResource(fileName);
			if (resourceURL != null) {
			...
			fileURL = GenerateLiberantContract.class.getResource(fileName);
		}
		if (fileURL == null) {
